...
`version` accepts an exact version (`11.5.0`), a range (`^11.0.0`, `~11.5.0`, `>=11.0.0`), or an npm dist-tag (`latest`). `version` and `version-file` are mutually exclusive. If neither is set, the version is read from `package.json` in the current directory if available, otherwise the latest version is installed.
