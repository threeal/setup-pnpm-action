...
- **`src/pnpm.test.ts`** — Tests for the pure functions in `pnpm.ts`: `getPnpmHome`, `getPnpmMajorVersion`, `getPnpmDownloadUrl` (pnpm <11), and `getPnpm11DownloadUrl` (pnpm >=11), including live network calls to verify URL accessibility.
